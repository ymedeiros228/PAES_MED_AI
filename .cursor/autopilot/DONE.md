...
- [x] **IV · Redação rascunho offline (modo A):** `essay_draft.dart` (SharedPreferences `essay_draft_v1`); restore no open, debounce 600 ms, limpa ao corrigir / Limpar rascunho; tema preservado vs race `_loadThemes`. Smoke `ciclo_iv_*`. Flutter: 10/10 unit+widget.
