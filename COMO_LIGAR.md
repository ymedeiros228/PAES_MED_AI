...
## Erros onda 4 + plano reforço (Ciclo CU)

1. Aulas / import review / flashcards: `humanApiError` (sem stack na UI).
2. Plano: card **Reforço do coach** (`MediaReinforcement` compact) no tópico do dia.
...
## Cards/Adaptativo Li batch (Ciclo DH)

1. Flashcards: batch reads + chip **Li** + `onMarkedRead`.
...
## Questões/Aprovar Li (Ciclo DL)

1. Lista Questões: batch reads + badge **Li** + ícone teoria + filtro ativo.
